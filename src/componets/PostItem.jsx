...
import { addLike } from '../redux/posts/postsOperation';
import defaultImage from '../images/user-foto-big.png';
import { useEffect } from 'react';

export default function PostItem({ post }) {
  const { id, image, title, comments, likes, location, locationName } = post;
  ...
  return (
    <View style={styles.postItem}>
      ...
      <View style={styles.info}>
        ...
        <TouchableOpacity
          style={styles.statItem}
          onPress={() => navigate('MapScreen', { locationName })}
        >
          <Feather name="map-pin" size={24} color="#BDBDBD" />
          <Text style={[styles.statText, styles.underline]}>{location}</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
